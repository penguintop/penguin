...
//
func (f Index) DumpAllKeys() ([]*Item, error) {
	it := f.db.NewIterator()
	defer it.Release()

	itemKeys := make([]*Item, 0)
	...
	for ok := it.Seek(f.prefix); ok; ok = it.Next() {
		...
		if key[0] != f.prefix[0] {
			...
			e, err := f.decodeKeyFunc(key)
			if err != nil {
				return nil, fmt.Errorf("decode key: %w", err)
			}
			eDump := new(Item)
			eDump.Address = make([]byte, 32)
			copy(eDump.Address, e.Address)
			itemKeys = append(itemKeys, eDump)
		}
	}
	return itemKeys, it.Error()
}
