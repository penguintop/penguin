...
func (a Address) MarshalText() ([]byte, error) {
	...
	t, err := HexAddrToXwcAddr(hex.EncodeToString(a[:]))
	if err != nil {
		return nil, err
	}
	return []byte(t), nil
...
func (a *Address) UnmarshalText(input []byte) error {
	addrHex, err := XwcAddrToHexAddr(string(input))
	if err != nil {
		return err
	}
...
func (a ConAddress) MarshalText() ([]byte, error) {
	t, err := HexAddrToXwcConAddr(hex.EncodeToString(a[:]))
	if err != nil {
		return nil, err
	}
	return []byte(t), nil
...
func (a *ConAddress) UnmarshalText(input []byte) error {
	addrHex, err := XwcConAddrToHexAddr(string(input))
	if err != nil {
		return err
	}
	addrBytes, _ := hex.DecodeString(addrHex)
...
func (a PubKey) MarshalText() ([]byte, error) {
	t, err := HexPubkeyToXwcPubkey(hex.EncodeToString(a[:]))
	if err != nil {
		return nil, err
	}
	return []byte(t), nil
...
func (a *PubKey) UnmarshalText(input []byte) error {
	pubkeyHex, err := XwcPubkeyToHexPubkey(string(input))
	if err != nil {
		return err
	}
	pubkeyBytes, _ := hex.DecodeString(pubkeyHex)
	a.SetBytes(pubkeyBytes)
	return nil
}
...
func (a *PubKeyType) UnmarshalText(input []byte) error {
	pubkeyBytes, _ := hex.DecodeString(string(input))
	a.SetBytes(pubkeyBytes)
	return nil
}
...
func (a UTCTime) MarshalText() ([]byte, error) {
	return []byte(property.UTCToRFC3339(uint64(a))), nil
}

func (a *UTCTime) UnmarshalText(input []byte) error {
	t, err := property.RFC3339ToUTC(string(input))
	if err != nil {
		return err
	}
	*a = UTCTime(t)
...
func (a *MemoMessage) UnmarshalText(input []byte) error {
	if len(input) <= 8 || string(input[0:8]) != "00000000" {
		return errors.New("invalid input")
	}
	m, err := hex.DecodeString(string(input[8:]))
	if err != nil {
		return err
	}
	*a = MemoMessage(m)
