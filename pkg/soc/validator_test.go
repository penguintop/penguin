	...
	for _, c := range []struct {
	...
	}{
		{
			...
			chunk: func() penguin.Chunk {
				...
				wrongAddress := penguin.NewAddress(wrongAddressBytes)
				return penguin.NewChunk(wrongAddress, sch.Data())
			},
		},
		{
			name: "invalid data",
			chunk: func() penguin.Chunk {
				...
				chunkData[0] = 0x01
				return penguin.NewChunk(socAddress, data)
			},
		},
		{
			name: "invalid id",
		...
		},
		{
			name: "nil data",
			chunk: func() penguin.Chunk {
				return penguin.NewChunk(socAddress, nil)
			},
		},
		{
			name: "small data",
			chunk: func() penguin.Chunk {
				return penguin.NewChunk(socAddress, []byte("small"))
			},
		},
		{
			name: "large data",
			chunk: func() penguin.Chunk {
				return penguin.NewChunk(socAddress, []byte(strings.Repeat("a", penguin.ChunkSize+penguin.SpanSize+1)))
			},
		},
	} {
