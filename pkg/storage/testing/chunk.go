...
import (
    ...
    penguintesting "github.com/penguintop/penguin/pkg/penguin/test"
)

var mockStamp penguin.Stamp
...
var fixtureChunks = map[string]penguin.Chunk{
	"0025": penguin.NewChunk(
		penguin.MustParseHexAddress("0025737be11979e91654dffd2be817ac1e52a2dadb08c97a7cef12f937e707bc"),
	...
	"0033": penguin.NewChunk(
		penguin.MustParseHexAddress("0033153ac8cfb0c343db1795f578c15ed8ef827f3e68ed3c58329900bf0d7276"),
	...
	"02c2": penguin.NewChunk(
		penguin.MustParseHexAddress("02c2bd0db71efb7d245eafcc1c126189c1f598feb80e8f14e7ecef913c6a2ef5"),
	...
	"7000": penguin.NewChunk(
		penguin.MustParseHexAddress("70002115a015d40a1f5ef68c29d072f06fae58854934c1cb399fcb63cf336127"),
...
func GenerateTestRandomChunk() penguin.Chunk {
...
// addressed chunk.
func GenerateTestRandomInvalidChunk() penguin.Chunk {
	data := make([]byte, penguin.ChunkSize)
	_, _ = rand.Read(data)
	key := make([]byte, penguin.SectionSize)
	_, _ = rand.Read(key)
	stamp := postagetesting.MustNewStamp()
	return penguin.NewChunk(penguin.NewAddress(key), data).WithStamp(stamp)
}

// GenerateTestRandomChunks generates a slice of random
// Chunks by using GenerateTestRandomChunk function.
func GenerateTestRandomChunks(count int) []penguin.Chunk {
...
func GenerateTestRandomChunkAt(target penguin.Address, po int) penguin.Chunk {
	data := make([]byte, penguin.ChunkSize)
	_, _ = rand.Read(data)
	addr := penguintesting.RandomAddressAt(target, po)
	stamp := postagetesting.MustNewStamp()
	return penguin.NewChunk(addr, data).WithStamp(stamp)
}

// FixtureChunk gets a pregenerated content-addressed chunk and
// panics if one is not found.
func FixtureChunk(prefix string) penguin.Chunk {
