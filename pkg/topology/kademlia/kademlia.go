...
import (
	...
	"github.com/penguintop/penguin/pkg/discovery"
...
func (k *Kad) connectionAttemptsHandler(ctx context.Context, wg *sync.WaitGroup, peerConnChan <-chan *peerConnInfo) {
	connect := func(peer *peerConnInfo) {
		penAddr, err := k.addressBook.Get(peer.addr)
		switch {
		...
		switch err = k.connect(ctx, peer.addr, penAddr.Underlay); {
		case errors.Is(err, errPruneEntry):
			k.logger.Debugf("kademlia: dial to light node with overlay %q and underlay %q", peer.addr, penAddr.Underlay)
			remove(peer)
			return
		case errors.Is(err, errOverlayMismatch):
			k.logger.Debugf("kademlia: overlay mismatch has occurred to an overlay %q with underlay %q", peer.addr, penAddr.Underlay)
			remove(peer)
			return
		case err != nil:
			k.logger.Debugf("kademlia: peer not reachable from kademlia %q: %v", penAddr, err)
...
func (k *Kad) connectBootnodes(ctx context.Context) {
	...
	for _, addr := range k.bootnodes {
		...
		if _, err := p2p.Discover(ctx, addr, func(addr ma.Multiaddr) (stop bool, err error) {
			...
			penAddress, err := k.p2p.Connect(ctx, addr)
			...
			if err := k.connected(ctx, penAddress.Overlay); err != nil {
				return false, err
			}
