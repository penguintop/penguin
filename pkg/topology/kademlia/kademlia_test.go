...
func TestAddressBookPrune(t *testing.T) {
	...
	*kademlia.TimeToRetry = 50 * time.Millisecond
	...
	nonConnPeer, err := pen.NewAddress(signer, nonConnectableAddress, test.RandomAddressAt(base, 1), 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStart(t *testing.T) {
	...
	t.Run("non-empty addressbook", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			...
			penAddr, err := pen.NewAddress(signer, multiaddr, peer, 0)
			if err != nil {
				t.Fatal(err)
			}
			if err := ab.Put(peer, *penAddr); err != nil {
				t.Fatal(err)
			}
		}
...
func p2pMock(ab addressbook.Interface, signer penCrypto.Signer, counter, failedCounter *int32) p2p.Service {
	p2ps := p2pmock.New(
		p2pmock.WithConnectFunc(func(ctx context.Context, addr ma.Multiaddr) (*pen.Address, error) {
			...
			penAddr, err := pen.NewAddress(signer, addr, address, 0)
			if err != nil {
				return nil, err
			}

			if err := ab.Put(address, *penAddr); err != nil {
				return nil, err
			}
