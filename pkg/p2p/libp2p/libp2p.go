...
import (
	...
	"github.com/penguintop/penguin/pkg/p2p/libp2p/internal/breaker"
...
func (s *Service) handleIncoming(stream network.Stream) {
	...
	if i.FullNode {
		err = s.addressbook.Put(i.PenAddress.Overlay, *i.PenAddress)
		if err != nil {
			...
			_ = s.Disconnect(i.PenAddress.Overlay)
			return
		}
	}
	...
	if s.notifier != nil {
		if !i.FullNode {
			...
			s.logger.Debugf("stream handler: notifier.Connected: peer disconnected: %s: %v", i.PenAddress.Overlay, err)
	...
	s.logger.Debugf("stream handler: successfully connected to peer %s%s (inbound)", i.PenAddress.ShortString(), i.LightString())
	s.logger.Infof("stream handler: successfully connected to peer %s%s (inbound)", i.PenAddress.Overlay, i.LightString())
}
...
func (s *Service) Connect(ctx context.Context, addr ma.Multiaddr) (address *pen.Address, err error) {
	...
	if i.FullNode {
		err = s.addressbook.Put(overlay, *i.PenAddress)
		if err != nil {
			_ = s.Disconnect(overlay)
			return nil, fmt.Errorf("storing pen address: %w", err)
		}
	}
	...
	s.logger.Debugf("successfully connected to peer %s%s (outbound)", i.PenAddress.ShortString(), i.LightString())
