...
import (
	...
	"github.com/penguintop/penguin/pkg/p2p/libp2p/internal/handshake/mock"
...
func TestHandshake(t *testing.T) {
	...
	node1PenAddress, err := pen.NewAddress(signer1, node1ma, addr, networkID)
	if err != nil {
		t.Fatal(err)
	}
	...
	node2PenAddress, err := pen.NewAddress(signer2, node2ma, addr2, networkID)
	if err != nil {
		t.Fatal(err)
	}

	node1Info := handshake.Info{
		PenAddress: node1PenAddress,
		FullNode:   true,
	}
	node2Info := handshake.Info{
		PenAddress: node2PenAddress,
		FullNode:   true,
	}
	...
	handshakeService, err := handshake.New(signer1, aaddresser, senderMatcher, node1Info.PenAddress.Overlay, networkID, true, nil, testWelcomeMessage, logger)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("Handshake - OK", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{
				Address: &pb.PenAddress{
					...
					Overlay:   node2PenAddress.Overlay.Bytes(),
					Signature: node2PenAddress.Signature,
				},
				NetworkID:      networkID,
				FullNode:       true,
		...
		if !bytes.Equal(ack.Address.Overlay, node1PenAddress.Overlay.Bytes()) ||
			!bytes.Equal(ack.Address.Underlay, node1maBinary) ||
			!bytes.Equal(ack.Address.Signature, node1PenAddress.Signature) ||
	...
	t.Run("Handshake - welcome message too long", func(t *testing.T) {
		...
		_, err := handshake.New(signer1, aaddresser, senderMatcher, node1Info.PenAddress.Overlay, networkID, true, nil, LongMessage, logger)
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{
				Address: &pb.PenAddress{
					Underlay:  node2maBinary,
					Overlay:   node2PenAddress.Overlay.Bytes(),
					Signature: node1PenAddress.Signature,
				},
				NetworkID: networkID,
				FullNode:  true,
			},
	...
	t.Run("Handle - OK", func(t *testing.T) {
		handshakeService, err := handshake.New(signer1, aaddresser, senderMatcher, node1Info.PenAddress.Overlay, networkID, true, nil, "", logger)
		if err != nil {
			t.Fatal(err)
		}
		var buffer1 bytes.Buffer
		...
		penAddress, err := pen.ParseAddress(got.Ack.Address.Underlay, got.Ack.Address.Overlay, got.Ack.Address.Signature, got.Ack.NetworkID)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Handle - transaction is not on the blockchain", func(t *testing.T) {
		...
		handshakeService, err := handshake.New(signer1, aaddresser, sbMock, node1Info.PenAddress.Overlay, networkID, true, []byte("0xff"), "", logger)
		if err != nil {
			t.Fatal(err)
		}
		var buffer1 bytes.Buffer
...
func testInfo(t *testing.T, got, want handshake.Info) {
	t.Helper()
	if !got.PenAddress.Equal(want.PenAddress) || got.FullNode != want.FullNode {
