...
func TestConnectDisconnect(t *testing.T) {
	...
	penAddr, err := s2.Connect(ctx, addr)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s2.Disconnect(penAddr.Overlay); err != nil {
		t.Fatal(err)
	}
...
func TestDoubleDisconnect(t *testing.T) {
	...
	if err := s2.Disconnect(penAddr.Overlay); !errors.Is(err, p2p.ErrPeerNotFound) {
...
func TestTopologyNotifier(t *testing.T) {
	...
	penAddr2, err := s1.Connect(ctx, addr2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s1.Disconnect(penAddr2.Overlay); err != nil {
		t.Fatal(err)
	}
	expectPeers(t, s1)
	expectPeersEventually(t, s2)
