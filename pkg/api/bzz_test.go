...
func TestPenFiles(t *testing.T) {
	var (
		fileUploadResource   = "/pen"
		targets              = "0x222"
		fileDownloadResource = func(addr string) string { return "/pen/" + addr }
...
func TestPenFilesRangeRequests(t *testing.T) {
...
func TestFeedIndirection(t *testing.T) {
	...
	jsonhttptest.Request(t, client, http.MethodPost, "/pen", http.StatusCreated, options...)
	...
	// called from the pen endpoint. then call the pen endpoint with
	...
	var (
		...
		penDownloadResource = func(addr, path string) string { return "/pen/" + addr + "/" + path }
	...
	jsonhttptest.Request(t, client, http.MethodGet, penDownloadResource(manifRef.String(), ""), http.StatusOK,
		jsonhttptest.WithExpectedResponse(updateData),
	)
}

func TestPenReupload(t *testing.T) {
	var (
		logger         = logging.New(ioutil.Discard, 0)
		mockStatestore = statestore.NewStateStore()
		m              = &mockSteward{}
	...
	jsonhttptest.Request(t, client, http.MethodPatch, "/v1/pen/"+addr.String(), http.StatusOK,
		jsonhttptest.WithExpectedJSONResponse(jsonhttp.StatusResponse{
