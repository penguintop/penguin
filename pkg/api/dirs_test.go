...
func TestDirs(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		verify := func(t *testing.T, resp api.PenUploadResponse) {
			t.Helper()
			...
			validateFile := func(t *testing.T, file f, filePath string) {
				...
				jsonhttptest.Request(t, client, http.MethodGet,
					penDownloadResource(resp.Reference.String(), filePath),
					http.StatusOK,
			...
			validateIsPermanentRedirect := func(t *testing.T, fromPath, toPath string) {
				...
				expectedResponse := fmt.Sprintf("<a href=\"%s\">Permanent Redirect</a>.\n\n",
					penDownloadResource(resp.Reference.String(), toPath))

				jsonhttptest.Request(t, client, http.MethodGet,
					penDownloadResource(resp.Reference.String(), fromPath),
			...
			validateAltPath := func(t *testing.T, fromPath, toPath string) {
				...
				jsonhttptest.Request(t, client, http.MethodGet,
					penDownloadResource(resp.Reference.String(), toPath), http.StatusOK,
					jsonhttptest.WithPutResponseBody(&respBytes),
				)

				jsonhttptest.Request(t, client, http.MethodGet,
					penDownloadResource(resp.Reference.String(), fromPath), http.StatusOK,
