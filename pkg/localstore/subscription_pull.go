		...
		for {
			...
			case <-out:
				...
				if err != nil {
					...
					db.logger.Debugf("out: localstore pull subscription iteration: bin: %d, since: %d, until: %d: %v", bin, since, until, err)
					return
				}
				if count > 0 {
					first = false
			...
			case <-ctx.Done():
				err := ctx.Err()
				if err != nil {
					db.logger.Debugf("done: localstore pull subscription iteration: bin: %d, since: %d, until: %d: %v", bin, since, until, err)
				}
				return
			}
		}
